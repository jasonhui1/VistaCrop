...
export interface CanvasViewProps {
  image: string | null
  onAddCrop: (cropData: CropData) => void
...
function CanvasView({ image, onAddCrop, onImageUpload }: CanvasViewProps) {
  ...
  const initialRotationRef = useRef<{ angle: number; startAngle: number }>({ angle: 0, startAngle: 0 })
  const [selection, setSelection] = useState<Selection | null>(null)
  const [selectionRotation, setSelectionRotation] = useState(0)
  const [imageSize, setImageSize] = useState<{ width: number; height: number }>({ width: 0, height: 0 })
  const [displaySize, setDisplaySize] = useState<DisplaySize>({ width: 0, height: 0, offsetX: 0, offsetY: 0 })
  ...
  const handleCreateCrop = () => {
    const rect = getSelectionRect()
    if (!rect || rect.width < 10 || rect.height < 10 || !displaySize.scale) return
