...
function FreeformCanvas({
    ...
    const handleItemMouseDown = useCallback((e, item, type = 'move') => {
        ...
        // Optimization: Cache crop lookup during mouse down so we don't do an O(N) array lookup during mouse move
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (dragState.type === 'corner' && dragState.cornerIndex !== undefined) {
            ...
            // Start from the customPoints saved during mousedown
            const currentPoints = startItem.customPoints ||
                (FRAME_SHAPES[startItem.frameShape]?.points || FRAME_SHAPES.rectangle.points).map(p => [...p])
            ...
            // Optimization: Remove setDragState call to prevent full component re-render on every mouse movement
            // delta is calculated as absolute difference from mousedown state (dragState.startX/startY)
