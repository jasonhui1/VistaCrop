...
// Precomputed static map for O(1) filter lookups
const FILTERS_MAP = new Map(FILTERS.map(f => [f.id, f]))
...
function FreeformCanvas({
    ...
    // Utility Functions
    // ========================================================================
    // Memoize crops array to O(1) map to avoid O(N) lookups in render and event loops
    const cropsMap = useMemo(() => new Map(crops.map(c => [c.id, c])), [crops])

    // Memoize placedItems array to O(1) map for faster lookups in mouse move events
    const placedItemsMap = useMemo(() => new Map(placedItems.map(item => [item.id, item])), [placedItems])
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (dragState.type === 'rotate') {
            ...
            const crop = cropsMap.get(startItem.cropId)
        ...
        if (dragState.type === 'crop-pan') {
            ...
            const item = placedItemsMap.get(dragState.itemId)
            if (!item) return
        ...
        if (dragState.type.startsWith('resize-')) {
            ...
            const crop = cropsMap.get(dragState.startItem.cropId)
            if (!crop) return
    ...
    }, [dragState, onUpdateItem, onUpdateItemSilent, cropsMap, placedItemsMap, composition.pageWidth, composition.pageHeight])
