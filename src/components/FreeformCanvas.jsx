...
function FreeformCanvas({
    ...
    const handleItemMouseDown = useCallback((e, item, type = 'move') => {
        ...
        const baseDragState = {
            ...
            startY: e.clientY,
            startItem: { ...item },
            startCrop: crops.find(c => c.id === item.cropId)
        }
        ...
        if (type === 'rotate' || type === 'frame-rotate') {
            ...
            if (type === 'rotate') {
                const crop = baseDragState.startCrop
    ...
    const handleCornerMouseDown = useCallback((e, item, cornerIndex) => {
        ...
        setDragState({
            ...
            startY: e.clientY,
            startItem: { ...item },
            currentPoints: item.customPoints || (FRAME_SHAPES[item.frameShape]?.points || FRAME_SHAPES.rectangle.points).map(p => [...p])
        })
    }, [])
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (dragState.type === 'rotate') {
            const { centerX, centerY, startAngle, startItem, startCrop: crop } = dragState
        ...
        if (dragState.type === 'crop-pan') {
            const { startItem, startCrop: crop } = dragState
            if (!crop) return
        ...
        if (dragState.type === 'corner' && dragState.cornerIndex !== undefined) {
            const { currentPoints, startItem } = dragState
            ...
            setDragState(prev => ({ ...prev, startX: e.clientX, startY: e.clientY, currentPoints: newPoints }))
        }
