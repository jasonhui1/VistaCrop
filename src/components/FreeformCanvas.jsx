...
function FreeformCanvas({
    ...
    const handleItemMouseDown = useCallback((e, item, type = 'move') => {
        ...
        // Base drag state
        // ⚡ Bolt: Cache O(N) `find()` lookups here on mouse down so we
        // can use O(1) property access during high-frequency mouse move events.
        const baseDragState = {
            type: actualType,
            itemId: item.id,
            startX: e.clientX,
            startY: e.clientY,
            startItem: { ...item },
            crop: crops.find(c => c.id === item.cropId)
        }
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (dragState.type.startsWith('resize-')) {
            ...
            const { crop, startItem } = dragState
            if (!crop) return
        ...
        if (dragState.type === 'corner' && dragState.cornerIndex !== undefined) {
            // ⚡ Bolt: Use `dragState.startItem` instead of searching `placedItems` on every tick.
