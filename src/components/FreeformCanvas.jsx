...
function FreeformCanvas({
    ...
    const handleItemMouseDown = useCallback((e, item, type = 'move') => {
        ...
        // Optimization: Cache initial crop and item to avoid O(N) array lookups during continuous high-frequency mouse move
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (dragState.type.startsWith('resize-')) {
            ...
            if (!dragState.startCrop) return

            const aspectRatio = dragState.startCrop.width / dragState.startCrop.height
        ...
        if (dragState.type === 'corner' && dragState.cornerIndex !== undefined) {
            const currentPoints = dragState.startItem.customPoints ||
                (FRAME_SHAPES[dragState.startItem.frameShape]?.points || FRAME_SHAPES.rectangle.points).map(p => [...p])

            const itemWidthPx = (dragState.startItem.width / composition.pageWidth) * rect.width
            const itemHeightPx = (dragState.startItem.height / composition.pageHeight) * rect.height
