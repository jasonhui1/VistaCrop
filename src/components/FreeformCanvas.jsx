...
function FreeformCanvas({
    ...
    const handleItemMouseDown = useCallback((e, item, type = 'move') => {
        ...
        // Base drag state
        // ⚡ Bolt: Cache associated crop to avoid O(N) lookup in handleMouseMove
        const startCrop = crops.find(c => c.id === item.cropId)
        const baseDragState = {
            type: actualType,
            itemId: item.id,
            startX: e.clientX,
        ...
        if (type === 'rotate' || type === 'frame-rotate') {
            ...
            if (type === 'rotate') {
                const currentRotation = item.rotation ?? startCrop?.rotation ?? 0
    ...
    const handleCornerMouseDown = useCallback((e, item, cornerIndex) => {
        ...
        // ⚡ Bolt: Cache initial polygon points to calculate absolute deltas in handleMouseMove
        const startPoints = item.customPoints ||
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (dragState.type === 'crop-pan') {
            ...
            // ⚡ Bolt: Use cached startItem instead of searching placedItems array
        ...
        if (dragState.type.startsWith('resize-')) {
            ...
            const aspectRatio = startCrop.width / startCrop.height
            const updates = calculateResizeUpdates(
                corner, deltaX, deltaY, startItem,
        ...
        if (dragState.type === 'corner' && dragState.cornerIndex !== undefined) {
            const { startItem, startPoints } = dragState

            // ⚡ Bolt: Use cached startItem instead of searching placedItems array
            const itemWidthPx = (startItem.width / composition.pageWidth) * rect.width
            const itemHeightPx = (startItem.height / composition.pageHeight) * rect.height

            // ⚡ Bolt: Calculate delta from original startX/Y to compute absolute new position
            const deltaPctX = ((e.clientX - dragState.startX) / itemWidthPx) * 100
            const deltaPctY = ((e.clientY - dragState.startY) / itemHeightPx) * 100

            // ⚡ Bolt: Apply delta to cached startPoints instead of tracking incremental changes
            const newPoints = startPoints.map((point, idx) => {
            ...
            // ⚡ Bolt: Removed continuous setDragState call that caused re-renders on every frame
        }
