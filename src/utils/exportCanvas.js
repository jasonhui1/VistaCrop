 ...
 */
import { FILTERS_MAP } from './filters'
...
async function exportPanelMode(ctx, composition, panels, crops) {
    for (const panel of panels) {
        ...
        ctx.save()
        // Performance optimization: Uses Map.get for O(1) lookup instead of O(N) Array.find
        ctx.filter = FILTERS_MAP.get(crop.filter)?.css || 'none'
        ctx.beginPath()
